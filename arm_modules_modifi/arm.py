    ...
    def __init__(self, robot_model, group_name="arm", gripper_name="gripper", robot_name=None, moving_time=2.0, accel_time=0.3, gripper_pressure=0.5, gripper_pressure_lower_limit=150, gripper_pressure_upper_limit=350, init_node=True):
        ...
        if gripper_name is not None:
            self.gripper = InterbotixGripperXSInterface(
                self.dxl, gripper_name, gripper_pressure, gripper_pressure_lower_limit, gripper_pressure_upper_limit)
...
class InterbotixArmXSInterface(object):
    ...
    def __init__(self, core, robot_model, group_name, moving_time=2.0, accel_time=0.3):
        ...
        self.initial_guesses = [
            [0.0] * self.group_info.num_joints for i in range(3)]
    ...
    def set_trajectory_time(self, moving_time=None, accel_time=None):
        if (moving_time is not None and moving_time != self.moving_time):
            ...
            self.core.srv_set_reg(cmd_type="group", name=self.group_name,
                                  reg="Profile_Velocity", value=int(moving_time * 1000))
        if (accel_time is not None and accel_time != self.accel_time):
            self.accel_time = accel_time
            self.core.srv_set_reg(cmd_type="group", name=self.group_name,
                                  reg="Profile_Acceleration", value=int(accel_time * 1000))
    ...
    def check_joint_limits(self, positions):
        ...
        speed_list = [abs(goal - current)/float(self.moving_time)
    ...
    def check_single_joint_limit(self, joint_name, position):
        ...
        speed = abs(
            theta - self.joint_commands[self.info_index_map[joint_name]])/float(self.moving_time)
    ...
    def set_ee_cartesian_trajectory(self, x=0, y=0, z=0, roll=0, pitch=0, yaw=0, moving_time=None, wp_moving_time=0.2, wp_accel_time=0.1, wp_period=0.05):
        ...
        for i in range(N+1):
            ...
            if success:
                ...
                rospy.loginfo("%.1f%% of trajectory successfully planned. Trajectory will not be executed." % (
                    i/float(N) * 100))
                break
        ...
        if success:
            ...
            self.core.pub_traj.publish(JointTrajectoryCommand(
    ...
    def get_ee_pose(self):
        joint_states = [self.core.joint_states.position[self.core.js_index_map[name]]
                        for name in self.group_info.joint_names]
        T_sb = mr.FKinSpace(
            self.robot_des.M, self.robot_des.Slist, joint_states)
        return T_sb
